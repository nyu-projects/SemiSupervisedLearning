...
def get_imgnet(location="./", batch_size=64, labels_per_class=100):
    from functools import reduce
    from operator import __or__
    from torch.utils.data.sampler import SubsetRandomSampler
    from torchvision.datasets import MNIST
    ...
    imgnet_train = MNIST(location, train=True, download=True,
                        transform=flatten_bernoulli, target_transform=onehot(n_labels))
    imgnet_valid = MNIST(location, train=False, download=True,
                        transform=flatten_bernoulli, target_transform=onehot(n_labels))
    ...
    labelled = torch.utils.data.DataLoader(imgnet_train, batch_size=batch_size, num_workers=2, pin_memory=cuda,
                                           sampler=get_sampler(imgnet_train.train_labels.numpy(), labels_per_class))
    unlabelled = torch.utils.data.DataLoader(imgnet_train, batch_size=batch_size, num_workers=2, pin_memory=cuda,
                                             sampler=get_sampler(imgnet_train.train_labels.numpy()))
    validation = torch.utils.data.DataLoader(imgnet_valid, batch_size=batch_size, num_workers=2, pin_memory=cuda,
                                             sampler=get_sampler(imgnet_valid.test_labels.numpy()))
...
if __name__ == "__main__":
    ...
    labelled = load_dataset(data_path="../../ssl_data_96/supervised/train/", batch_size=100) 
    unlabelled = load_dataset(data_path="../../ssl_data_96/unsupervised/", batch_size=100)
    validation = load_dataset(data_path="../../ssl_data_96/supervised/val/", batch_size=100)
    ...
    models += [DeepGenerativeModel([224*224*3, n_labels, 50, [600, 600]])]
